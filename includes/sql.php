...
/*--------------------------------------------------------------*/
/* Function for Update lease from table by id
/*--------------------------------------------------------------*/
function update_single_table_colummn($table, $id, $col, $val)
{
  global $db;
  if (tableExists($table)) {
    $sql = "UPDATE " . $db->escape($table);
    $sql .= " SET " . $db->escape($col) . "=" . $db->escape($val);
...
/*--------------------------------------------------------------*/
/* Find all leases by
  /* Joining leases table and products table
  /*--------------------------------------------------------------*/
function find_all_leases()
{
  global $db;
  $results = array();
  $sql = "SELECT l.id,l.date,l.fullname,l.department,l.product_id,l.number,l.reason,l.status,";
  $sql .= "p.name ";
  $sql .= "FROM leases l ";
  $sql .= "LEFT JOIN products p ";
  $sql .= "ON p.id=l.product_id ORDER BY l.date ASC";
  $result = find_by_sql($sql);
  return $result;
}
/*--------------------------------------------------------------*/
